...
export interface Client {
  ...
  skipConsent?: boolean;
  /**
   * The user ID of the client owner.
   */
  userId?: string;
}
...
type DbTypeToTsType<T extends string> = T extends 'string'
  ? string
  : T extends 'boolean'
    ? boolean
    : T extends 'date'
...
// Extract a single field type
type ExtractField<T> = T extends { type: infer U extends string } ? DbTypeToTsType<U> : unknown;

// Convert fields object to TypeScript interface
type ExtractFields<T> = {
  [K in keyof T as IsOptional<T[K]> extends true ? never : K]: ExtractField<T[K]>;
} & {
  [K in keyof T as IsOptional<T[K]> extends true ? K : never]?: ExtractField<T[K]>;
};

// Main utility type
export type InferFromPluginSchema<T> = T extends { fields: infer F } ? ExtractFields<F> : never;
