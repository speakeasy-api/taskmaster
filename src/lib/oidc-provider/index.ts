...
const getJwtPlugin = (ctx: GenericEndpointContext) => {
  return ctx.context.options.plugins?.find((plugin) => plugin.id === 'jwt') as ReturnType<
    typeof jwt
  >;
};
...
export const getMetadata = (ctx: GenericEndpointContext, options?: OIDCOptions): OIDCMetadata => {
...
export const oidcProvider = (options: OIDCOptions) => {
   ...
   */
  async function storeClientSecret(ctx: GenericEndpointContext, clientSecret: string) {
  ...
  return {
    ...
    endpoints: {
      ...
      oAuth2token: createAuthEndpoint(
        ...
        async (ctx) => handleTokenRequest(ctx, options)
      ),
      oAuth2userInfo: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const baseUserClaims = {
            ...
            given_name: requestedScopes.includes('profile') ? user.name.split(' ')[0] : undefined,
            family_name: requestedScopes.includes('profile') ? user.name.split(' ')[1] : undefined,
            email_verified: requestedScopes.includes('email') ? user.emailVerified : undefined
          };
          const userClaims = options.getAdditionalUserInfoClaim
            ? await options.getAdditionalUserInfoClaim(user, requestedScopes, client)
            : baseUserClaims;
          return ctx.json({
            ...baseUserClaims,
            ...userClaims
      ...
      registerOAuthApplication: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const clientSecret =
            options.generateClientSecret?.() || generateRandomString(32, 'a-z', 'A-Z');
