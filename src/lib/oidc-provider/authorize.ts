...
function getErrorURL(ctx: GenericEndpointContext, error: string, description: string) {
...
export async function authorize(ctx: GenericEndpointContext, options: OIDCOptions) {
  ...
  if (!query.client_id) {
    const errorURL = getErrorURL(ctx, 'invalid_client', 'client_id is required (none passed)');
  ...
  if (!client) {
    const errorURL = getErrorURL(ctx, 'invalid_client', 'client_id is required (not found)');
    throw ctx.redirect(errorURL);
  }
  ...
  if (
    ...
    return handleRedirect(
      formatErrorURL(query.redirect_uri, 'invalid_request', 'invalid code_challenge method')
    );
  }
  ...
  try {
    ...
    console.error('Error saving verification value:', e);
    return handleRedirect(
      formatErrorURL(
        query.redirect_uri,
        'server_error',
